...
class Remove_Instructor implements ActionListener{
    JFrame f;
    JTextField t;
    ...
    Remove_Instructor(){
        f=new JFrame("Remove Instructor");
       ...
       l5=new JLabel();
        ...
        l5.setLayout(null);
        ImageIcon img=new ImageIcon(ClassLoader.getSystemResource("online/course/registration/icons/remove.jpg"));
        l5.setIcon(img);
        f.add(l5);
    ...
    public void actionPerformed(ActionEvent ae){
        ...
        if(ae.getSource()==b){
            try{
                //connecting to database and searching for instructor details with instructur id from user
                conn con = new conn();
                String str = "select ins_spec,ins_name,ins_exp from instructor where ins_id ='"+t.getText()+"' ";
                ResultSet rs = con.s.executeQuery(str);

                int i=0;
                if(rs.next()){
                    String cour_type = rs.getString(1);
                    String cour_dur = rs.getString(2);
                    String cour_name=rs.getString(3);
                    ...
                    b2.setVisible(true);    
                    i=1;
                    l6.setText(cour_type);
                    l7.setText(cour_dur);
                    l8.setText(cour_name);
                }
                if(i==0)
        ...
        if(ae.getSource()==b1){
            try{
                //connecting to database and deleting instructor details taking instructor id from user
                conn con = new conn();
                String str = "delete from instructor where ins_id = '"+t.getText()+"'";
